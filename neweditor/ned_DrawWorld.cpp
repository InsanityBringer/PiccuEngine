...
int SelectRoomsInBox(int left, int top, int right, int bot)
{
	...
	RendHandle* WhyEvenHaveTheParameterToBeginWith = last_vp;

	Draw_handle = WhyEvenHaveTheParameterToBeginWith;

	//vp->clear();

	StartEditorFrame(*Draw_handle, &last_viewer_position, &last_view_orient, last_zoom);
...
bool WireframeFindRoomFace(RendHandle& handle, int x, int y, int* roomnum, int* facenum, int find_mode, bool one_room)
{
	...
	StartEditorFrame(handle, &last_viewer_position, &last_view_orient, last_zoom);
...
void DrawWorld(RendHandle& handle, vector* view_target, matrix* view_orient, float view_dist, int start_room, float rad, int drawflags, prim* prim)
{
	...
	//Draw_viewport = vp;
	Draw_handle = &handle;
	StartEditorFrame(handle, &viewer_position, view_orient, zoom);
