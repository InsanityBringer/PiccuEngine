...
public:
	...
	virtual void Render();
	void InitCamera();
	void DrawOrthoGrid(CMemDC_NED *pDC,int size);
	void DrawOrthoWorld(CMemDC_NED *pDC);
	void DrawOrthoRefFrame(CMemDC_NED *pDC);
	void DrawOrthoObjects(CMemDC_NED *pDC,room *rp,object *obj,int num_objects);
	void DrawRoomCenter(CMemDC_NED *pDC,room *rp,const int color);
	void DrawOrthoRoom(CMemDC_NED *pDC,room *rp,const int color);
	void DrawOrthoFace(CMemDC_NED *pDC,face *fp,room *rp,const int color);
	void DrawOrthoEdge(CMemDC_NED *pDC,face *fp,room *rp,const int color);
	void DrawOrthoFaceNormal(CMemDC_NED *pDC,room *rp,int facenum,const int color);
	void DrawOrthoObjectMark(CMemDC_NED *pDC,room *rp,object *objp,const int color);
	void DrawOrthoGamePaths(CMemDC_NED *pDC,room *rp,const int color,const int br_color);
	void DrawOrthoAIPaths(CMemDC_NED *pDC,room *rp,const int color,const int br_color);
	void DrawOrthoNumber(CMemDC_NED *pDC,int num,vec2D pos,float size,const int color);
	vec2D GetCursorPos();
	void SetCursorPos(vec2D pos);
	void MoveCursor(float amount_x, float amount_y);
	void DrawCursor(CMemDC_NED *pDC);
	void CenterRoom(room *rp);
	void CenterFace(room *rp,int facenum);
	void CenterOrigin();
	void DrawVerts(CMemDC_NED *pDC,vector *verts,int num_verts,const int color);
	void DrawCurrentPrims(CMemDC_NED *pDC);
	void DrawMarkedPrims(CMemDC_NED *pDC);
