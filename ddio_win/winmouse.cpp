...
int ddio_MouseGetState(int* x, int* y, int* dx, int* dy, int* z, int* dz)
{
	...
	if (dx) *dx = (int)ceil(abs(DDIO_mouse_state.dx) * (double)MouseScalar) * (DDIO_mouse_state.dx < 0 ? -1 : 1);
	if (dy) *dy = (int)ceil(abs(DDIO_mouse_state.dy) * (double)MouseScalar) * (DDIO_mouse_state.dy < 0 ? -1 : 1);
